...
echo "POST Install chaincode on Org1 CHANGED ORGANIZATION"
echo
curl -s -X POST \
...
echo "POST Install chaincode on Org2 CHANGED ORGANIZATION"
echo
curl -s -X POST \
  http://localhost:4000/chaincodes \
...
echo
curl -s -X POST \
  http://localhost:4000/channels/mychannel/chaincodesUpdate \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json" \
  -d "{
	\"chaincodeName\":\"mycc\",
